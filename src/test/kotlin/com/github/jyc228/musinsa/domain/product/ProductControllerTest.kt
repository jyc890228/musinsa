...
class ProductControllerTest : IntegrationTest() {
    ...
    fun `product 를 등록할 수 있다`() {
        client.createProduct(brandId, categoryIds[0], 1000.toBigInteger()) shouldBeGreaterThan 0
    }

    @Test
    fun `product 를 수정할 수 있다`() {
        val pid = client.createProduct(brandId, categoryIds[2], 1000.toBigInteger())

        shouldNotThrowAny { client.updateProduct(pid, brandId, categoryIds[2], 2000.toBigInteger()) }
        shouldNotThrowAny { client.updateProduct(pid, brandId, categoryIds[3], 2000.toBigInteger()) }

        shouldThrow<MusinsaApiClient.ResponseException> {
            client.updateProduct(pid + 394, brandId, 2, 2000.toBigInteger())
        }.statusCode shouldBe 404
        shouldThrow<MusinsaApiClient.ResponseException> {
            client.updateProduct(pid, brandId, categoryIds[2], 0.toBigInteger())
        }.statusCode shouldBe 400
    }

    @Test
    fun `product 를 삭제할 수 있다`() {
        val pid = client.createProduct(brandId, categoryIds[5], 1000.toBigInteger())
